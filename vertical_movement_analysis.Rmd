...
Remove the max depth bathymetry at ca. 600 m depth. I am interested in the vertical movement behaviour on the continental shelf, which is max 300 m deep. Hence, a bathymetry of ca. 600 m indicates the eel was in the Atlantic Ocean beyond the shelf
```{r}
data_1eel <- filter(data_1eel, max_depth_bathymetry >= -300)
...
```{r, fig.cap = "Daily max bathymetry (obtained via geolocation) for 1 eel."}
ggplot(data_1eel, aes(x = datetime,
                                 y = max_depth_bathymetry)) +
  geom_point() +
  theme_minimal() +
  ylab("Depth (m)") +
  xlab("Date") +
...
```{r, fig.cap = "Vertical movement pattern over the daily max bathymetry for 1 eel."}
...
for (i in 2:dim(data_1eel)[1]){
  if (data_1eel$sun_altitude[i] > data_1eel$sun_altitude[i-1]){
    data_1eel$sun_class[i] = "sunrise"
  } else{
    data_1eel$sun_class[i] = "sunset"
  }}
