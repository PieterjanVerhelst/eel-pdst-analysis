...
### Construct GLMM TMB with AR1 covariance structure
```{r}
subset <- subset %>%
  arrange(ID, datetime) %>%
  group_by(ID) %>%
...
The response variable 'rel_depth' contains values > 1. Still need to figure out why. Untill then, I just remove the values > 1
```{r}
