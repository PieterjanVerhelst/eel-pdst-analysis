...
```{r}
forecast::Acf(subset$corrected_depth, type = c("correlation"), lag.max=max(dim(subset)), plot = TRUE)  
```

Next, create the ACF plot for the first 500 observations.
```{r}
forecast::Acf(subset$corrected_depth, type = c("correlation"), lag.max=500, plot = TRUE)
```
...
```{r}
vals_list <- forecast::Acf(subset$corrected_depth, type = c("correlation"), lag.max=max(dim(subset)), plot = FALSE)
...
```
```{r}
summary(subset$corrected_depth)
```
```{r}
...
```{r}
ggplot(subset, aes(x = datetime,
                                   y = -1*corrected_depth_abs)) +
  geom_line() +
  theme_minimal() +
  ylab("Depth (m)") +
  xlab("Date") +
...
```{r}
subset2 <- filter(subset,datetime >= "2019-02-01 00:00:00", datetime <= "2019-02-11 00:00:00")
...
```{r}
ggplot(subset, aes(x = temperature,
                                 y = -1*rel_depth)) +
  geom_point() +
  theme_minimal() +
  ylab("Relative depth (m)") +
...
```{r}
ggplot(subset2, aes(x = temperature,
                                 y = -1*rel_depth)) +
  geom_point() +
  theme_minimal() +
  ylab("Relative depth (m)") +
...
```{r}
ggplot(subset, aes(x = max_depth_bathymetry,
                                 y = -1*corrected_depth_abs)) +
  geom_point() +
  theme_minimal() +
  ylab("Depth (m)") +
...
```{r}
ggplot(subset, aes(x = direction_x,
                                       y = -1*rel_depth)) +
  geom_point() +
  theme_minimal() +
  ylab("Relative depth (m)") +
...
```{r}
ggplot(subset2, aes(x = direction_x,
                                       y = -1*rel_depth)) +
  geom_point() +
  theme_minimal() +
  ylab("Relative depth (m)") +
...
```{r}
ggplot(subset, aes(x = direction_y,
                                   y = -1*rel_depth)) +
  geom_point() +
  theme_minimal() +
  ylab("Relative depth (m)") +
...
```{r}
ggplot(subset2, aes(x = direction_y,
                                   y = -1*rel_depth)) +
  geom_point() +
  theme_minimal() +
  ylab("Relative depth (m)") +
...
```{r}
ggplot(subset, aes(x = 100*moon_fraction,
                                   y = -1*rel_depth)) +
  geom_point() +
  theme_minimal() +
  ylab("Relative depth (m)") +
...
```{r}
ggplot(subset2, aes(x = 100*moon_fraction,
                                   y = -1*rel_depth)) +
  geom_point() +
  theme_minimal() +
  ylab("Relative depth (m)") +
...
```{r}
ggplot(subset, aes(x = sun_altitude,
                                   y = -1*rel_depth)) +
  geom_point() +
  theme_minimal() +
  ylab("Relative depth (m)") +
...
```{r}
ggplot(subset2, aes(x = sun_altitude,
                                   y = -1*rel_depth)) +
  geom_point() +
  theme_minimal() +
  ylab("Relative depth (m)") +
...
```{r}
ggplot(subset, aes(x = sun_azimuth,
                                   y = -1*rel_depth)) +
  geom_point() +
  theme_minimal() +
  ylab("Relative depth (m)") +
...
```{r}
ggplot(subset2, aes(x = sun_azimuth,
                                   y = -1*rel_depth)) +
  geom_point() +
  theme_minimal() +
  ylab("Relative depth (m)") +
...
## Conduct GLMM TMB model

Next, a GLMM TMB model was applied to a subset of 5 eels with a similar migration path (through the Channel). I want to apply this complex model first on only a few fish before scaling up the complexity. For more info on the method implementation, see [this website](https://cran.r-project.org/web/packages/glmmTMB/vignettes/covstruct.html). The [glmmTMB ](https://cran.r-project.org/web/packages/glmmTMB/vignettes/glmmTMB.pdf) R-package was used.
...
### Construct GLMM with AR1 covariance structure
```{r}
subset <- subset %>%
  arrange(ID, datetime) %>%
  group_by(ID) %>%
...
When running the glme() function, it gives an error. Not sure why, but based on fora searches it might have to do with the data complexity and/or size.
```{r}
...
The function bam from the mgcv package is designed for large datasets.
