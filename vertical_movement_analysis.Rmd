...
```{r}
subset2 <- filter(subset,datetime >= "2019-02-01 00:00:00", datetime <= "2019-02-06 00:00:00")
...
### Plot vertical depth pattern over eastward current velocity vector
```{r}
...
### Plot vertical depth pattern over northward current velocity vector
```{r}
...
### Plot vertical depth pattern over day and night
...
### Plot vertical depth pattern over illuminated moon phase
```{r}
...
In case it would be needed, the respone variable is detrended by subtracting a value by the previous value.
```{r}
subset <- subset %>% 
  group_by(ID) %>%
  arrange(datetime) %>%
  mutate(diff_depth = corrected_depth_abs - lag(corrected_depth_abs))

```
```{r}
par(mfrow=c(2,1))
plot(subset$datetime, subset$corrected_depth_abs)
plot(subset$datetime, subset$diff_depth)
```

Check ACF & PACF plot if data is stationary
```{r}
forecast::Acf(subset$diff_depth, type = c("correlation"), lag.max=20, plot = TRUE)
```

```{r}
forecast::Pacf(subset$diff_depth, lag.max=20, plot = TRUE)
```

## Conduct ARIMA model

```{r}
fit <- Arima(subset$corrected_depth_abs, order=c(8,1,0))
fit
```
