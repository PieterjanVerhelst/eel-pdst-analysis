...
sensordata <- read_csv("./data/interim/sensorlogs/sensor_A17668_10-05-2021.csv")
...
release <- as.POSIXct("2020-11-18 11:10:00", "%Y-%m-%d %H:%M:%S", tz = "GMT")
retrieval <- as.POSIXct("2021-03-20 23:59:00", "%Y-%m-%d %H:%M:%S", tz = "GMT") # Take day before retrieval, since exact moment of retrieval is unknown
...
subset2 <- filter(aggdata, 
                  datetime == as.POSIXct("2020-11-18 10:55:00", "%Y-%m-%d %H:%M:%S", tz = "GMT") |
                  datetime == as.POSIXct("2021-01-11 20:30:00", "%Y-%m-%d %H:%M:%S", tz = "GMT"))
...
press$regression <- ( -2.447e-06          *press$numericdate) + 3.930e+03
...
# 7. Write csv files ####
write.csv(temp, "./data/interim/geolocation_input_files/input_A17668/EELA17668TEMP.csv", row.names = FALSE)
write.csv(press, "./data/interim/geolocation_input_files/input_A17668/EELA17668PRES.csv", row.names = FALSE)
