...
sensordata <- read_csv("./data/interim/sensorlogs/sensor_A17634_19-03-2021.csv")
...
release <- as.POSIXct("2020-12-22 16:15:00", "%Y-%m-%d %H:%M:%S", tz = "GMT")
retrieval <- as.POSIXct("2021-01-14 23:59:00", "%Y-%m-%d %H:%M:%S", tz = "GMT") # Take day before retrieval, since exact moment of retrieval is unknown
...
subset2 <- filter(aggdata, 
                  datetime == as.POSIXct("2020-12-22 16:00:00", "%Y-%m-%d %H:%M:%S", tz = "GMT") |
                  datetime == as.POSIXct("2021-01-03 12:44:00", "%Y-%m-%d %H:%M:%S", tz = "GMT"))
...
press$regression <- (3.171e-06       *press$numericdate)    -5.097e+03    
...
# 7. Write csv files ####
write.csv(temp, "./data/interim/geolocation_input_files/input_A17634/EELA17634TEMP.csv", row.names = FALSE)
write.csv(press, "./data/interim/geolocation_input_files/input_A17634/EELA17634PRES.csv", row.names = FALSE)
