...
sensordata <- read_csv("./data/interim/sensorlogs/sensor_A17521_08-03-2022.csv")
...
retrieval <- as.POSIXct("2021-06-05 23:59:00", "%Y-%m-%d %H:%M:%S", tz = "GMT") # Take day before retrieval, since exact moment of retrieval is unknown
...
subset2 <- filter(aggdata, 
                  datetime == as.POSIXct("2020-12-24 12:35:00", "%Y-%m-%d %H:%M:%S", tz = "GMT") |
                  datetime == as.POSIXct("2020-12-29 02:33:00", "%Y-%m-%d %H:%M:%S", tz = "GMT"))
...
press$regression <- (  -1.263e-06          *press$numericdate)  + 2.033e+03
...
# 7. Write csv files ####
write.csv(temp, "./data/interim/geolocation_input_files/input_A17521_2/EELA17521TEMP.csv", row.names = FALSE)
write.csv(press, "./data/interim/geolocation_input_files/input_A17521_2/EELA17521PRES.csv", row.names = FALSE)
