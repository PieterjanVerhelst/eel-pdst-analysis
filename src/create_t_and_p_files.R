...
sensordata <- read_csv("./data/interim/sensorlogs/sensor_A17449_02-03-2022.csv")
...
retrieval <- as.POSIXct("2021-08-07 23:59:00", "%Y-%m-%d %H:%M:%S", tz = "GMT") # Take day before retrieval, since exact moment of retrieval is unknown
...
subset2 <- filter(aggdata, 
                  ...
                  datetime == as.POSIXct("2021-02-06 07:04:00", "%Y-%m-%d %H:%M:%S", tz = "GMT"))
...
press$regression <- (  -1.893e-06          *press$numericdate)  + 3.050e+03
...
# 7. Write csv files ####
write.csv(temp, "./data/interim/geolocation_input_files/input_A17449/EELA17449TEMP.csv", row.names = FALSE)
write.csv(press, "./data/interim/geolocation_input_files/input_A17449/EELA17449PRES.csv", row.names = FALSE)
